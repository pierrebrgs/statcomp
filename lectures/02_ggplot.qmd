...
ggplot(diamonds, aes(x=carat, y=price)) +
  ...
  scale_x_log10() + scale_y_log10() #échelle logarithmique, pour chaque
```

### 2d histogram

```{r}
ggplot(diamonds, aes(x=carat, y=price)) + #diamonds is for the dataset choice
  geom_bin2d() +
...
ggplot(diamonds) + 
  geom_bar(aes(x = cut, y=after_stat(prop), group=1)) +
  scale_y_continuous(labels = scales::percent_format()) + #make the label in %
...
```{r, eval=FALSE}
p = ggplot(mtcars, aes(mpg, wt)) + geom_point() #"save" ggplot in p, it won't be showed, just stored
png("mtcars.png")  #use pdf("mtcars.pdf") for a pdf format  # it opens a file
print(p)
dev.off() #close the plotting device (saves the file) #it closes the file
```
...
```{r}
ggcorrmat(hc[,-1]) #remove the first column of the hc (hatecrime) dataset 
```
...
```{r}
library(gganimate)
library(gifski)  #you may need this in Windows and Mac #if doesn't work use tools
gini = as_tibble(read.csv("C:/Users/Pierre/OneDrive/Bureau/M1 applied eco - UPF/T2/modern statistical computing/github/statcomp/datasets/gini.csv"))
gini
```
...
```{r}
ggplot(gini, aes(x=Year , y=Gini)) + geom_line(aes(colour=Entity), show.legend = TRUE)
```

2. Add `transition_reveal(Year)` to add an animation over time

```{r}
a = ggplot(gini, aes(x=Year , y=Gini)) + geom_line(aes(colour=Entity), show.legend = TRUE) + transition_reveal(Year)
animate(a, renderer= gifski_renderer())
#anim_save("animcountries.gif", animation = last_animation())
```
