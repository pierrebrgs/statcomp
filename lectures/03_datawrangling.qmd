...
The data are stored in a `tibble`, an extension of `data.frame` that prints nicer and shows some info: #doesn't work if the data are stored in a matrix
...
Other functions: `rename`, `transmute` #transmute creates a NEW dataset with the data you added


## Filter {auto-animate="true"}

```{r}
jan1= filter(flights, month == 1, day == 1) #both conditions will be met, c'est un et et non un ou
jan1
```
...
```{r}
jan1= filter(flights, month == 1 & day == 1) #la même chose, pour montrer qu'on peut utiliser d'autres signes ig
jan1
```
...
```{r}
sel= (flights[,'month']==1) & (flightsm[,'day']==1) #sel will be full of TRUE, FALSE, ...
flightsel= flightsm[sel,] #when sel is true select row (before coma), and no condition on the columns so select all of them
flightsel[1:5,1:3]
```
...
x= NA
x == 20 #is Mary 20? #ça teste si les 2 sont égaux
...
filter(df, is.na(x) | x > 1) #la barre c'est un ou
```
...
```{r}
arrange(flights, year, month, day) #arrange by year, then by month, then by day in an increasing way
```
...
```{r}
select(flights, -year, -month,-day) #exclude year, month and day from this dataset
```
...
```{r}
select(flights, time_hour, air_time, everything()) #je crois que c'est everything else
```
...
```{r}
rename(flights, deptime = dep_time) #new = old
```
...
We select a subset of columns and create 2 new ones #mutate means adding column
...
```{r}
flights_sml= select(flights, year:day, contains("delay"), distance, air_time) #years:day select every column between year and day
...
## Alternative to mutate: base R

Some find it easier to create variables separately, then add it to the tibble
...
Let's do the exercise with and without dplyr
...
### No mutate
```{r}
uaflight= (flights$carrier == 'UA') #vector of true or false
...
finedep[finedep<0]= 0 #no reward for arriving early
...
Package `skimr` provides a nice function `skim()` to compute basic stats for all variables in a dataset. `skim_with()` has further customization options
...
```{r}
aggregate(dep_delay ~ year + month + day, data=flights, FUN=mean, na.rm=TRUE) #removing missing values
```
...
delay= summarise(by_dest, count = n(), #n() will count how many flights go to that airport
...
#by continent
continents= group_by(gapminder, continent)
summarise(continents, avgLE = mean(lifeExp, na.rm = TRUE), avgGDPpp = mean(gdpPercap, na.rm = TRUE))
#by year
years = group_by(gapminder, year)
summarise(years, avgLE = mean(lifeExp, na.rm = TRUE), avgGDPpp = mean(gdpPercap, na.rm = TRUE))

#question 2
a = ggplot(continent_summary, aes(x = mean_gdpPercap, y = mean_lifeExp, color = continent)) + geom_point() + labs(title = 'Year: {frame_time}', x = 'average GDP per capita', y = 'average life expectancy') +  transition_time(year) 
...
```{r}
#question 1
summarise(years, inter_quartile_range = IQR(gdpPercap))

#question 2
b = summarise(years, inter_quartile_range = IQR(gdpPercap), gdpPercap)
ggplot(b, aes(x=year, y=inter_quartile_range)) + geom_point() + geom_line() + labs(x="year", y="IQR of GDP per Capita")
